...
namespace Genisis
{
    ...
    static class Backend
    {
        ...
        {
            Process.Start("https://www.google.co.nz/search?q=" + SearchFormatter(command));
        }
    
        public static string SearchFormatter( RecognitionResult searchPhrase )
        {
            string searchQuery = "";
            for (int i = 2; i < searchPhrase.Words.Count; i++)
            {
                searchQuery += searchPhrase.Words[i].Text + "+";
            }
            return searchQuery;
        }
    }
